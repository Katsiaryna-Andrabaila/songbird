...
export function getMainResults() {
    ...
    if(scoreNum < 30) {
        if(language === 'ru') resultsInfo.textContent = `Вы прошли викторину и набрали ${scoreNum} из 30 возможных баллов`;
        if(language === 'en') resultsInfo.textContent = `You completed the quiz and scored ${scoreNum} out of 30 possible points`;
    } else if (scoreNum === 30) {
        if(language === 'ru') resultsInfo.textContent = `Вы прошли викторину, набрали ${scoreNum} из 30 возможных баллов и стали абсолютным чемпионом! Вы МОЛОДЧИНА! Держите конфетку:`;
        if(language === 'en') resultsInfo.textContent = `You passed the quiz, scored ${scoreNum} out of 30 possible points and became the absolute champion! You are YOUTH! Hold the candy:`;
